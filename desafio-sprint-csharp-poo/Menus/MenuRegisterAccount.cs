...
namespace BedrockBankCorp.Menus
{
    internal class MenuRegisterAccount : Menu
    {
        public override void Execute(List<ContaBancaria> registeredAccounts)
        {
            ...
            if (type == "1")
                registeredAccounts.Add(new ContaPoupanca(holder, password));
            else if (type == "2")
                registeredAccounts.Add(new ContaCorrente(holder, password));
            else if (type == "3")
                registeredAccounts.Add(new ContaEmpresarial(holder, password, 5000m));
            else
